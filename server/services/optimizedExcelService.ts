    ...
    for (let i = 0; i < chunkData.length; i++) {
      try {
        ...
        if (globalProcessedVoterIds.has(hashedVoterId)) {
          errors.push(`Row ${actualRowNumber}: Duplicate VoterID ${voterIdStr}`);
          duplicates++;
          continue;
        }
    ...
    try {
      ...
      const parseExcelDate = (value: string | number | undefined): string | null => {
        ...
        if (typeof value === 'number') {
          // Excel date serial number - account for Excel's 1900 leap year bug
          // Excel incorrectly treats 1900 as a leap year, so we need to adjust
          let adjustedValue = value;
          if (value >= 60) {
            adjustedValue = value - 1; // Subtract 1 day for dates after Feb 28, 1900
          }

          // Convert from Excel serial date to JavaScript date
          const excelEpoch = new Date('1899-12-30T00:00:00Z'); // Excel day 1 = 1900-01-01
          const date = new Date(excelEpoch.getTime() + adjustedValue * 24 * 60 * 60 * 1000);
      ...
      return {
        systemId,
        fullName,
        voterId: voterIdStr, // Store voter ID
        firstName,
        lastName,
        middleName,
